...
// desafio 7
app.get('/crush/search', tokenValidation, async (req, res) => {
  const file = await fs.readFile(fileName);
  const crushes = JSON.parse(file);
  const searchTerm = req.query.q;

  if (searchTerm === '' || searchTerm === undefined) return res.status(200).json(crushes);

  const filterCrushes = crushes.filter((crush) => crush.name.includes(searchTerm));
  if (filterCrushes === []) return res.status(200).json([]);
  return res.status(200).json(filterCrushes);
});
